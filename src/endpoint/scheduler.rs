...
impl Drop for JobHandle {
    fn drop(&mut self) {
        info!("Cleaning up JobHandle");
        if self.container_id.is_some() {
            info!("Container already created");
            ...
            tokio::spawn(async move {
                ...
                if container_info.state.running {
                    info!("Container is still running, cleaning up...");
                    ...
                    info!("Container has already finished");
                }
            });
        } else {
            info!("No container created");
        }
    }
}
