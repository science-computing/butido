...
pub fn cli<'a>() -> Command<'a> {
    Command::new("butido")
        ...
        .subcommand(Command::new("source")
            ...
            .subcommand(Command::new("link_check")
                .version(crate_version!())
                .about("Check whether the source links of a package (or multiple) is still valid")
                .arg(Arg::new("package_name")
                    .required(false)
                    .multiple(false)
                    .index(1)
                    .value_name("PKG")
                    .about("Check the source URL for this package")
                )
                .arg(Arg::new("package_version")
                    .required(false)
                    .multiple(false)
                    .index(2)
                    .value_name("VERSION")
                    .about("Check the source URL for the package in this version")
                )
                ...
                .arg(Arg::new("matching")
                    ...
                    .takes_value(true)
                    .value_name("REGEX")
                    .about("Check all packages matching a regex with their name")
                )

                .group(ArgGroup::new("check-one-or-many")
                    .args(&["package_name", "matching"])
                    .required(true)
                )
            )
